...
def ranking_nodes(single_node, max_emi, weight_emi, weight_cons, weight_com):
    emissions = round(
        single_node["carbon_intensity"] * single_node["energy_consumption"], 2
    )  # Quantity of emission per hour
    score = (
        (weight_emi * (1 - (emissions / max_emi)))
        + (weight_cons * single_node["consistency"])
        + (weight_com * single_node["completeness"])
    )
...
def nodes_selection(
    ...
    for node in data:
        emissions.append(
            round(node["carbon_intensity"] * node["energy_consumption"], 2)
        )
    ...
    data_volume_node = round(((1.0 / n_total_clients) * data_volume), 2)
    ...
    for obj_exp in data_sorted:
        if data_volume_node < obj_exp["data_volume"]:
    ...
    nodes = nodes_data_suc[:n_clients]
...
def nodes_selection_smart(
    ...
    data_volume_total = round((data_volume_node * n_clients), 2)
    ...
    active_nodes = nodes_data_suc[:n_clients]
    # List of future candidates
    candidates = nodes_data_suc[n_clients:]
    ...
    for node in active_nodes:
        ...
        print(f"Final percentage of node {node['id']} is {final_percentage}")
    ...
    while data_volume_sum < data_volume_total:
        ...
        obj = candidates[i]
        if (data_volume_total - data_volume_sum) < obj["data_volume"]:
            final_percentage_second = compute_effective_percentage(
                obj, x_train, y_train, x_val, y_val, indexes_train, indexes_val
            )
...
def nodes_selection_smart_minimal(
    ...
    for node in active_nodes:
        ...
        if percentage > 0.0:
            ...
            percentages.append(percentage)
            ultimate_nodes.append(nodes_data_suc[n_clients])
