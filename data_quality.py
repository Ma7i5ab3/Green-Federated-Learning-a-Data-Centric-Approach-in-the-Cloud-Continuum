...
def clean_dataset(dataset_percent, x_train, y_train, extend):
  ...
  if current_consistency < 1:
    ...
    repeats = filter(lambda x: x.shape[0] > 1, pd.DataFrame(x_train).groupby([0]).indices.values())
...
def compute_effective_percentage(node, x_train, y_train, x_val, y_val, indexes_train, indexes_val):
  (x_train, y_train), (x_test, y_test) = load_partition(node['id'], x_train, y_train, x_val, y_val, indexes_train, indexes_val)
  initial_dataset_length = len(x_train)
  #Retrieve the dataset in the original form
  options_consistency = {'data_quality_dimension_percentage': node['consistency'], 'experiment_method': 'uniform'}
  options_completeness = {'data_quality_dimension_percentage': node['completeness'], 'experiment_method': 'uniform'}
  x_train_d, y_train_d = reduce_completeness_nan(x_train, y_train, options_completeness)
  ...
  final_percentage_node = round(((node['data_volume']*final_dataset_length) / initial_dataset_length), 2)
